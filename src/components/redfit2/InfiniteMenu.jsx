...
function createProgram(gl, shaderSources, transformFeedbackVaryings, attribLocations) {
  ...
  if (attribLocations) for (const a in attribLocations) gl.bindAttribLocation(program, attribLocations[a], a);
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    const sketch = new InfiniteGridMenu(
      canvas,
      list,
      (index) => setActiveItem(list[index % list.length]),
      setIsMoving,
      sk => sk.run(),
      scale
    );
  ...
  return (
    <div className="infinite-menu-wrap">
      ...
      {activeItem && (
        <>
          <h2 className={`face-title ${stateClass}`}>{activeItem.title}</h2>
          <p className={`face-description ${stateClass}`}>{activeItem.description}</p>
          {activeItem.link ? (
            <button
              type="button"
              onClick={handleButtonClick}
              className={`action-button ${stateClass}`}
              aria-label={activeItem.title || 'Open'}
            >
